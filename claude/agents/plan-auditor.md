...
- **Separate durable from disposable.** Tag each decomposed item: DURABLE = intent, user-visible behavior, hard constraints (security, data integrity, API contracts, repo rules); DISPOSABLE = implementation choices the plan happened to name (a specific library, file layout, variable names). Hold the code strictly to DURABLE items. For DISPOSABLE ones, an equivalent-outcome deviation is NOT drift: note it as an intentional deviation, do not verdict it `contradicts`. Reserve `contradicts` for an unmet DURABLE item or a violated hard constraint; plans go stale on detail, intent and constraints are what must hold.
