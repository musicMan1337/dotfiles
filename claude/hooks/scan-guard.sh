...
#
# Rules are evaluated PER SEGMENT, never against the whole command string.
# (2026-08-14: `rm -rf x; mkdir -p x; ls | grep -v y` was denied as a recursive
# grep, because the -r came from rm and the grep came from a later pipeline
# stage; they were never the same command. Identical class to the unanchored rm
# bug fixed in bash-egress-guard.sh, commit e03c4c6. A flag or path may only arm
# a rule for the command it actually belongs to.)
#
...
# Start of a segment: optional whitespace, then any leading VAR=value
# assignments, then the command word itself. Because segments are already split
# on separators, this is the segment's own command, so `git grep -r` (git is the
# command) and `docker ps | grep foo` args cannot false-match.
CMDSTART='^[[:space:]]*([A-Za-z_][A-Za-z0-9_]*=[^[:space:]]*[[:space:]]+)*'

# Recursive-grep detection, scoped to this segment: grep/egrep/fgrep as the
# segment's command AND an -r/-R flag cluster or --recursive in that same
# segment. Misses `xargs grep -r` shapes, but those are bounded by the
# (guarded) find/glob that feeds them.
grep_r() {
  sm "${CMDSTART}(grep|egrep|fgrep)[[:space:]]" \
    && sm "[[:space:]](-[A-Za-z]*[rR][A-Za-z]*([[:space:]]|$)|--(dereference-)?recursive)"
}

# Split on list/pipeline separators and subshell parens. Doubles (&&, ||) just
# yield an extra empty segment, which is skipped. Command substitution `$(rg ...)`
# splits too, so the inner command is still evaluated on its own.
while IFS= read -r seg; do
  [ -z "${seg//[[:space:]]/}" ] && continue

  # --- 1. Any scan rooted at home / root / /Users: always too broad ------------
  if sm "${CMDSTART}(rg|fd|find)[[:space:]]" || grep_r; then
    if sm "[[:space:]](~|\\\$HOME|/Users(/derek)?|/)([[:space:]]|$)"; then
  ...
  if sm "${CMDSTART}rg[[:space:]]" \
     && sm "[[:space:]](--no-ignore(-[a-z]+)?|--hidden|--unrestricted|-u{1,3})([[:space:]]|$)"; then
  ...
  if sm "${CMDSTART}fd[[:space:]]" \
     && sm "[[:space:]](--no-ignore(-[a-z]+)?|--hidden|--unrestricted|-[HIu])([[:space:]]|$)"; then
  ...
  if sm "${CMDSTART}find[[:space:]]" \
     && ! sm "\-(i?name|i?path|i?regex|maxdepth|mindepth|newer|newermt|mmin|mtime|samefile|inum|prune|empty)\b"; then
