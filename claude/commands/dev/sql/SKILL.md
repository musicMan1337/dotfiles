...
- **Archive dir for retired scripts:** `/Users/derek/eBacon/SQL/queries/cc_done/`. `queries/` as a whole is gitignored, so both trees are scratch. See "Retiring scripts" below.

### Retiring scripts: `cc/` is live, `cc_done/` is finished

`queries/cc/` holds ONLY scripts still relevant to in-flight work. Anything finished moves to `queries/cc_done/`, preserving its subpath (`cc/tests/x.sql` → `cc_done/tests/x.sql`, `cc/cases/y.sql` → `cc_done/cases/y.sql`). Move, never delete: a retired script is still the record of what was run.

A script is finished when its branch is merged (or abandoned), its case is closed, its RESULTS block has been parsed into a verdict, or it was a one-off diagnostic whose question is answered. A script is NOT finished while its branch is unmerged, its baseline is still awaiting a post reading, or it is a reusable harness/reference the workflow points at (e.g. the parity-differential reference implementation).

Rules:

- **`git mv` is wrong here** (gitignored, untracked): use `mv`. Create the destination subdir first (`mkdir -p`).
- **Never move on your own initiative.** Propose the list of candidates with the reason each looks finished, and move only what the user confirms.
- **Reads still search both.** When looking for a prior script (a past case's test, an earlier diagnostic), check `cc_done/` too before concluding it does not exist.
- **New scripts are always written to `cc/`**, never straight into `cc_done/`.
...
2. **Differential harness** (real engine): one self-contained script in `queries/cc/tests/`, named `<caseid>_<slug>_parity_differential.sql`. Reference implementation ships with this skill: `examples/parity-differential.example.sql` (the 355075 NESCO harness, frozen; read it, don't run it).
...
- **`cc/` stays live; retire to `cc_done/`.** Finished scripts (merged branch, closed case, verdict parsed, question answered) move to `queries/cc_done/` with their subpath preserved, by `mv` and only with the user's confirmation. Search both dirs when hunting for a prior script.
