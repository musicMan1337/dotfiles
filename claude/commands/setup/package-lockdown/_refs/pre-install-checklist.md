...
| `ERR_PNPM_EXOTIC_SUBDEP` for `<pkg>` (resolved via git-repository) | A transitive declares its dep by git URL/commit instead of a registry version. Check if the same package is published: `npm view <pkg> version`. (Seen: `@electron/rebuild` -> `@electron/node-gyp` by git commit; the fork IS on npm.) | If published: add `<pkg>: "<registry-version>"` to `pnpm-workspace.yaml#overrides` so it resolves from the signed registry; verify publish date is past cooldown. If NOT published: decide deliberately — vendoring, or accepting the git source via config — never disable `blockExoticSubdeps` globally. (Snout bootstrap, 2026-06-09.) |
