...
A T-SQL script **written to a file** at `~/eBacon/SQL/queries/test-instructions/<case#>.sql`. The script:
...
**File path convention:** `~/eBacon/SQL/queries/test-instructions/<case#>.sql`. Create the `test-instructions/` directory if it doesn't exist (the parent `queries/` always does in the SQL repo). The filename is just the bare case number — matches the loose convention of files in `queries/` (e.g. `AR_317314.sql`). If a file already exists for that case, ask before overwriting.
...
-- ════════════════════════════════════════════════════════════════════
-- SQLChange (SPs)  — INCLUDE ONLY IF THE BRANCH HAS SP CHANGES
-- ════════════════════════════════════════════════════════════════════
INSERT INTO @ProcResult
EXEC dbo.createProcess
    @location         = @LocSql,
...
- **Only emit a process block per type that has actual changes.** If the branch only touches the UI/PHP, emit only the `ViperChange` block. If only SPs changed, emit only `SQLChange`. Don't pad the script with empty buckets — the user explicitly removes them when they only want one type.

## Process

1. Determine the case number (branch parse, fall back to asking the user).
2. Determine which processType buckets to produce — `ViperChange` if there are FE/PHP commits, `SQLChange` if there are SP commits, both if both. Use `git log master..HEAD` in each repo to decide. **If the user explicitly scopes to one bucket (e.g. "Viper only"), emit only that one.**
...
4. Ensure `~/eBacon/SQL/queries/test-instructions/` exists, then **write the script to `~/eBacon/SQL/queries/test-instructions/<case#>.sql`**. If a file already exists for that case, ask the user whether to overwrite, append, or skip.
5. In chat, summarize what was written: file path, how many process records, total step count per processType. Don't dump the full script back — the file is the deliverable. Include the one-line cleanup hint: `UPDATE t_ViperProcess SET voidDate = GETDATE() WHERE item = '<case#>' AND itemType = 'Case' AND voidDate IS NULL;` for stale re-runs.
6. Mention which DB the user should target (typically `Stage`) and that re-running appends rows unless they void first.

The script file is the primary output. Don't paste the full SQL into chat unless the user explicitly asks.
