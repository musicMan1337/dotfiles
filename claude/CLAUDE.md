...
Goal: keep the main session's context lean (premium 1M model; every token in context is re-billed each turn) while staying under the AV concurrency ceiling below. Delegate to subagents when a search or analysis is broad, parallelizable, or would dump bulky raw results into context (log sweeps, codebase exploration, doc research). Direct Glob/Grep/Read is fine for targeted lookups where you know roughly where to look; a delegated one-line grep costs more than a direct one.
